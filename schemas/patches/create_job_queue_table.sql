...
-- Function: Get next pending job, reclaiming abandoned processing jobs
...
BEGIN
  -- Jobs stuck in processing with no retries left are abandoned, not queued again
  UPDATE job_queue
  SET status = 'failed',
  ...
  -- Pending jobs, or processing jobs abandoned by a backgrounded/closed tab
...
-- Claim a specific job (webhook worker). Concurrent callers skip locked rows.

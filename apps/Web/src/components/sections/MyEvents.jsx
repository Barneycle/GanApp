...
export const MyEvents = () => {
  ...
  const [isUnregistering, _setIsUnregistering] = useState(false);
  ...
  const handleUnregister = async () => {
    ...
    const removed = registeredEvents.find((event) => event.id === eventId);
    setUnregisterDialog({ isOpen: false, eventId: null, eventTitle: '' });
    const result = await runOptimistic({
      apply: () => setRegisteredEvents((prev) => prev.filter((event) => event.id !== eventId)),
      request: () => EventService.unregisterFromEvent(eventId, user.id),
      revert: () => {
        if (removed) setRegisteredEvents((prev) => [removed, ...prev]);
      },
    });
    if (result?.error) {
  ...
  if (error) {
    return (
      <ErrorState
        error={error}
        context="loadMyEvents"
        onRetry={loadRegisteredEvents}
      />
    );
  }
  ...
  const _eventStatus = getEventStatus(featuredEvent);
                    ...
                    <div className="p-4 sm:p-6">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <button
                          ...
                          className={`w-full px-4 py-2 rounded-lg transition-colors text-sm ${canTakeSurvey
                        ...
                        <button
                          ...
                          className={`w-full px-4 py-2 rounded-lg transition-colors text-sm ${canGenerateCert

...
import { Camera, X, Info, ChevronDown, Search } from 'lucide-react';
...
import { inlineError, toErrorCopy } from '../../utils/errorCopy';
...
export const SetupProfile = () => {
  ...
  useEffect(() => {
    if (user && !hasInitializedForm.current && organizations.length > 0) {
      setFormData({
        prefix: user.prefix || '',
      ...
      // Check if user's organization exists in the loaded organizations list
  ...
  const handleOrgSearchChange = (e) => {
    ...
    // If user types and it doesn't match any org, show custom input option
    if (value && !organizations.some(group => 
      group.organizations.some(org => 
        org.name.toLowerCase().includes(value.toLowerCase())
      )
    )) {
      // Don't auto-switch to custom, just show dropdown with "Other" option
    }
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-4 sm:p-6 lg:p-8">
          <form onSubmit={handleSubmit}>
            ...
            <div className="space-y-6 mb-6">
              ...
              <div>
                ...
                <input
                  ...
                  disabled={loading}
                />
                <CharCount value={formData.first_name} max={FIELD_LIMITS.firstName} />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  disabled={loading}
                />
                <CharCount value={formData.last_name} max={FIELD_LIMITS.lastName} />
              </div>
                    ...
                    <div className="relative" ref={orgDropdownRef}>
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-10 pr-10 py-3 border border-slate-300 rounded-xl bg-slate-50 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent"
            ...
            <button
              type="submit"
              disabled={!canSubmit}
              className={`w-full py-3 rounded-xl font-semibold text-base shadow-lg transition-colors ${
                canSubmit
            ...
            >
              {loading ? (
                <SmartSpinner active variant="inline" light label="Completing setup" messages={['Still saving', 'Almost there']}>
                  Complete Setup
                </SmartSpinner>
              ) : (
                'Complete Setup'

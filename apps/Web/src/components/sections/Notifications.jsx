...
export const Notifications = () => {
  ...
  const handleMarkAsRead = async (notificationId) => {
    await runOptimistic({
      apply: () => setNotifications((prev) =>
        prev.map((n) => (n.id === notificationId ? { ...n, read: true } : n))
      ),
      request: () => NotificationService.markAsRead(notificationId),
      revert: () => setNotifications((prev) =>
  ...
  const handleMarkAllAsRead = async () => {
    ...
    await runOptimistic({
      apply: () => setNotifications((prev) => prev.map((n) => ({ ...n, read: true }))),
      request: () => NotificationService.markAllAsRead(user.id),
      revert: () => setNotifications(snapshot),
    });
  };
  ...
  const handleDelete = async (notificationId) => {
    ...
    await runOptimistic({
      apply: () => setNotifications((prev) => prev.filter((n) => n.id !== notificationId)),
      request: () => NotificationService.deleteNotification(notificationId),
      revert: () => {
  ...
  const _handleDeleteAllRead = async () => {
    if (!user?.id) return;
  ...
  const handleDeleteAll = async () => {
    ...
    const confirmed = await confirmDialog({
      ...
      message: 'This will permanently delete every notification in your inbox.',
  ...
  const handleDeleteSelected = async () => {
    ...
    const confirmed = await confirmDialog({
      ...
      message: `This will delete ${selectedNotifications.length} notification${selectedNotifications.length === 1 ? '' : 's'}.`,
      confirmText: 'Delete',
      type: 'danger',
    });
    if (!confirmed) return;
    ...
    setSelectedNotifications([]);
    notify('success', 'Selected notifications deleted');
  };

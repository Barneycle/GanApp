...
import { Calendar, MapPin, Clock, ArrowLeft, Star, CircleCheck } from 'lucide-react';
import { motion } from 'framer-motion';
import { PageSkeleton } from '../loading/Skeleton';
import { SmartSpinner } from '../loading/SmartSpinner';
import { confirmDialog, statusError } from '../Toast';
...
export const Evaluation = () => {
  ...
  const canSubmitSurvey = !validateForm() && !submitting;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: _data, error: submitError } = await supabase
        .from('survey_responses')
        .insert([{
          survey_id: survey.id,
          user_id: user.id,
      ...
      await statusError(err, 'submitSurvey');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const renderQuestion = (question, index) => {
    ...
    const hasValue = Array.isArray(currentResponse)
      ? currentResponse.length > 0
      : Boolean(String(currentResponse).trim());
    const hasStarted = Object.values(responses).some((value) => (
      Array.isArray(value) ? value.length > 0 : Boolean(String(value || '').trim())
    ));
    const showMissing = isRequired && !hasValue && hasStarted;
    ...
    switch (questionType) {
        ...
        return (
          <div key={questionId} className="mb-6">
            ...
            <input
              ...
              className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:border-transparent ${showMissing ? 'border-red-400 focus:ring-red-500' : 'border-slate-300 focus:ring-blue-500'}`}
              placeholder="Type your answer here..."
              required={isRequired}
            />
            {showMissing ? <FieldError error="This question needs an answer." /> : null}
          </div>
        );
        ...
        return (
          <div key={questionId}>
            ...
            <textarea
              ...
              onChange={(e) => handleResponseChange(questionId, e.target.value)}
              rows={4}
              className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:border-transparent resize-none ${showMissing ? 'border-red-400 focus:ring-red-500' : 'border-slate-300 focus:ring-blue-500'}`}
      ...
      case 'star-rating':
        const _starMin = question.scaleMin || 1;
  ...
  if (!isAuthenticated || user?.role !== 'participant') {
    return (
      <ErrorState
        error={errorCopy({
          what: !isAuthenticated ? 'You need to sign in.' : "You can't submit this survey.",
          why: !isAuthenticated
            ? 'Surveys are only available to participants who are logged in.'
            : 'Only participants can fill out event surveys.',
          action: !isAuthenticated
            ? 'Sign in with your participant account, then open this survey again.'
            : 'Go back to My events, or open this survey while signed in as a participant.',
        })}
        onRetry={() => navigate(!isAuthenticated ? '/login' : '/my-events')}
        retryLabel={!isAuthenticated ? 'Go to Login' : 'Back to My Events'}
      />
    );
  }
  ...
  if (!survey) {
    return (
      <ErrorState
        error={errorCopy({
          what: 'No survey is available.',
          why: 'This event does not have a survey open yet.',
          action: 'Go back to My events. If you just attended, wait until the organizer opens the survey.',
        })}
        onRetry={() => navigate('/my-events')}
        retryLabel="Back to My Events"
      />
    );
  }
            ...
            <button
              onClick={async () => {
                const confirmed = await confirmDialog({
                  title: 'Clear saved responses?',
                  message: 'This will erase all saved draft answers for this evaluation. This cannot be undone.',
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-100 p-4 sm:p-6 lg:p-8">
          <form onSubmit={handleSubmit}>
            {survey.questions && survey.questions.length > 0 ? (() => {
              ...
              survey.questions.forEach((question, _index) => {
            ...
            <div className="mt-8">
              {!canSubmitSurvey && survey?.questions?.length ? (
                <p className="mb-3 text-sm text-slate-500">Answer every required question to submit.</p>
              ) : null}
              ...
              <button
                ...
                className="flex-1 px-6 py-3 bg-blue-900 text-white rounded-lg hover:bg-blue-800 transition-all duration-200 font-medium disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"
              >
                <span className="flex items-center justify-center min-h-[1.5rem]">
                  <SmartSpinner
                    ...
                    variant="inline"
                    light
                    label="Submitting survey"
                    messages={['Still submitting', 'Almost there']}
                  >
                    Submit Survey
                  </SmartSpinner>
                </span>

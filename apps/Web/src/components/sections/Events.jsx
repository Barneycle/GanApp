...
export const Events = () => {
  ...
  const [registeringEvents, _setRegisteringEvents] = useState(new Set());
  ...
  const handleSetFeatured = async (eventId) => {
    try {
      ...
      if (result.error) {
        ...
        statusDialog({
          title: 'Now featuring',
          message: 'This event is featured on the home page.',
        });
      }
    } catch (error) {
      await statusError(error, 'generic', {
  ...
  const confirmRegistration = async () => {
    ...
    if (isSampleEvent) {
      ...
      statusDialog({
        ...
        message: 'This is a demo registration. Real events will appear in My events.',
      });
    ...
    try {
      ...
      const previousRegistrations = userRegistrations;
      const previousDetails = userRegistrationDetails;
      ...
      const result = await runOptimistic({
        ...
        request: async () => EventService.registerForEvent(eventId, user.id),
        revert: () => {
          setUserRegistrations(previousRegistrations);
          setUserRegistrationDetails(previousDetails);
        },
      });

      if (result?.error) {
        const confirmed = await statusError(result.error, 'register');
        if (confirmed && /already registered/i.test(String(result.error))) {
          navigate('/my-events');
        }
        return;
      }
      statusDialog({
        title: "You're registered",
        message: 'This event is now in My events. We’ll remind you as the date gets closer.',
      });
      ...
      const confirmed = await statusError(err, 'register');
      if (confirmed && /already registered/i.test(String(err?.message || err))) {
        navigate('/my-events');
      }
    } finally {
      setEventToRegister(null);
  ...
  const handleToggleRegistration = async (eventId, currentStatus) => {
    ...
    const result = await runOptimistic({
      apply: () => {
        setSelectedEvent((prev) => (prev ? { ...prev, registration_open: newStatus } : prev));
        setEvents((prev) => prev.map((event) => (
          event.id === eventId ? { ...event, registration_open: newStatus } : event
        )));
      },
      request: () => EventService.toggleEventRegistration(eventId, newStatus),
      revert: () => {
        setSelectedEvent(previousEvent);
        setEvents((prev) => prev.map((event) => (
          event.id === eventId ? { ...event, registration_open: currentStatus } : event
        )));
      },
    });
  ...
  const submitCancellationRequest = async () => {
    ...
    if (!cancellationReason.trim() || !cancellationDate) {
      return;
    }
    ...
    try {
      ...
      if (result.error) {
        ...
        statusDialog({
          ...
          message: 'An admin will review your cancellation request.',
        });
      ...
      await statusError(err, 'generic', { confirmText: 'Try again' });
    } finally {
  ...
  if (error && (activeTab !== 'archived' ? events.length === 0 : archivedEvents.length === 0)) {
    return (
      <ErrorState
        error={error}
        context="loadEvents"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {displayEvents.map((event, index) => {
                  ...
                  return (
                    <motion.div
                      key={event.id}
                      className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 border border-slate-100 overflow-hidden"
                      {...cardEnter(index)}
                      whileHover={{ y: -4 }}
                    >
                      {/* Event Banner */}
        ...
        {showAddCheckInModal && selectedParticipantForCheckIn && selectedEvent && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-4 sm:p-6">
              ...
              {error && (
                <ErrorBanner error={error} context="checkIn" className="mb-4" />
              )}
              <div className="flex space-x-3">
                <button
                  onClick={() => {
...
const _toDateInputValue = (value) => {
  if (!value) return '';
  return value.split('T')[0] || '';
};

const _toTimeInputValue = (value) => {
  if (!value) return '';
...
const _extractDateTimeParts = (value) => {
  if (!value) {
    return { date: '', time: '' };
  }

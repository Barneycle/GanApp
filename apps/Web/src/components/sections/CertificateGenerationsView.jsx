...
import { X, CircleCheck, CircleX, Calendar } from 'lucide-react';
...
export const CertificateGenerationsView = ({ isOpen, onClose, event }) => {
  ...
  const _loadJobQueueCertificates = async (eventId) => {
    try {
  ...
  const _orphanedCertificates = databaseCerts.filter(cert => {
    if (!cert.user_id) return false; // Skip certs without user_id
    return !participantIds.has(cert.user_id);
  });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} panelClassName="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          ...
          {!loading && error && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6 flex items-start">
              <CircleX className="w-5 h-5 text-red-600 mr-3 mt-0.5" />
            ...
            <>
              ...
              {participants.length === 0 ? (
                <div className="text-center py-12">
                  <CircleX className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {participants.map((participant) => {
                          ...
                          const participantUser = participant.users || participant;
                          const participantName = `${participantUser.first_name || ''} ${participantUser.last_name || ''}`.trim() || participantUser.email || 'Participant';
                          const _nameKey = participantName ? `name_${participantName.toLowerCase().trim()}` : null;
                            ...
                            <tr key={participantId} className="hover:bg-gray-50 transition-colors">
                              <td className="px-4 py-3 whitespace-nowrap">
                                {hasCertificate ? (
                                  ...
                                  <div className="flex items-center">
                                    <CircleX className="w-5 h-5 text-orange-500 mr-2" />

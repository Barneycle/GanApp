...
export const Settings = () => {
  ...
  const handlePreferenceChange = async (key, value) => {
    ...
    const result = await runOptimistic({
      apply: () => setPreferences((prev) => ({ ...prev, [key]: value })),
      request: () => SettingsService.updateNotificationPreferences(user.id, { ...previous, [key]: value }),
      revert: () => setPreferences(previous),
    });
    if (result?.error) {
      setError(result.error);
    } else {
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-100 p-6 mb-6">
          ...
          <div className="mt-6 flex justify-end">
            <button
              onClick={handleSave}
              disabled={saving}
              className="px-6 py-2 bg-blue-900 text-white rounded-lg hover:bg-blue-800 transition-colors font-medium flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {saving ? (

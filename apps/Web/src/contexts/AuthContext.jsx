      ...
      async (event, session) => {
        ...
        if (!isInitializing.current) {
          if (event === 'SIGNED_IN' && session) {
            // Skip re-processing if this is the same user re-confirming on tab focus,
            // not an actual new sign-in (Supabase fires SIGNED_IN on visibility).
      ...
      if (result.user) {
        ...
        if (result.error) {
          ...
          if (result.errorType === 'email') {
            formattedError = "This email isn't registered.";
          } else if (result.errorType === 'password') {
            formattedError = "The password doesn't match.";

...
describe('CertificateJobProcessor', () => {
  ...
  beforeEach(() => {
    ...
    (CertificateService.getNextCertificateNumber as any) = vi.fn().mockResolvedValue({
      number: 'CERT-001',
      error: null,
    });
    (CertificateService.incrementCertificateCounter as any) = vi.fn();
    (CertificateService.getCurrentCertificateCount as any) = vi.fn();
  ...
  describe('processCertificateJob', () => {
    it('should process certificate job successfully', async () => {
      ...
      expect(CertificateService.getNextCertificateNumber).toHaveBeenCalledWith('event-123', 'CERT');
      expect(CertificateService.getCurrentCertificateCount).not.toHaveBeenCalled();
      expect(CertificateService.incrementCertificateCounter).not.toHaveBeenCalled();
      expect(generatePNGCertificate).toHaveBeenCalledWith(
        expect.anything(),
        'CERT-001',
        expect.objectContaining({ participantName: 'John Doe' }),
      );
    });
    ...
    it('should abort if reserving a certificate number fails', async () => {
      (CertificateService.getCertificateConfig as any).mockResolvedValue({
        config: { ...mockConfig, cert_id_prefix: 'CERT' },
        error: null,
      });
      (CertificateService.getNextCertificateNumber as any).mockResolvedValue({
        number: undefined,
        error: 'RPC error',
      });
      ...
      expect(result.success).toBe(false);
      expect(result.error).toMatch(/reserve certificate number/i);
      expect(generatePNGCertificate).not.toHaveBeenCalled();
      expect(CertificateService.saveCertificate).not.toHaveBeenCalled();
    });

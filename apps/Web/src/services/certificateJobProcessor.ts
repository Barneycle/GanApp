...
export class CertificateJobProcessor {
  ...
  static async processCertificateJob(jobData: CertificateGenerationJobData): Promise<{
    ...
    try {
      ...
      // Reserve the certificate number atomically BEFORE generating files.
      // get_next_certificate_number locks/upserts the counter row so concurrent
      // jobs cannot read the same value and collide on certificate_number UNIQUE.
      ...
      if (config.cert_id_prefix) {
        if (eventId === 'standalone') {
          ...
          LoggerService.serviceLog('CertificateJobProcessor', 'Reserving certificate number for event', { eventId: eventIdForCounter });
          const numberResult = await CertificateService.getNextCertificateNumber(eventIdForCounter, config.cert_id_prefix);
          if (numberResult.error || !numberResult.number) {
            LoggerService.serviceError('CertificateJobProcessor', 'Failed to reserve certificate number', undefined, { error: numberResult.error });
            return {
              success: false,
              error: `Failed to reserve certificate number: ${numberResult.error || 'No number returned'}`
            };
          }
          certificateNumber = numberResult.number;
          LoggerService.serviceLog('CertificateJobProcessor', 'Reserved certificate number', { certificateNumber });
        }
      } else {

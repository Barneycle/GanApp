...
export class CertificateService {
  ...
  static async saveCertificate(
    ...
    return LoggerService.time('CertificateService.saveCertificate', async () => {
      ...
      let _existing: { certificate?: Certificate; error?: string } = { certificate: undefined };
  ...
  static async incrementCertificateCounter(eventId: string): Promise<{ success?: boolean; error?: string }> {
    try {
      ...
      const { data: _data, error } = await supabase.rpc('get_next_certificate_number', {
        event_uuid: eventId
      });
  ...
  /**
   * Reserve the next certificate number for an event (prefix + auto-incrementing counter).
   * Calls get_next_certificate_number, which atomically upserts the counter so concurrent jobs cannot share a number.
   */

...
export class EventService {
  ...
  static async checkUserSurveyCompletion(eventId: string, userId: string): Promise<{ isCompleted: boolean; error?: string }> {
    try {
      ...
      const { data: surveyResponses, error: _surveyError } = await supabase
      ...
      const { data: evaluationResponses, error: _evalError } = await supabase
  ...
  static async updateCheckInValidation(
    ...
    try {
      ...
      const { data: updatedRows, error: updateError, count: _count } = await supabase
        .from('attendance_logs')
        .update(updateData)
        .eq('id', checkInId)

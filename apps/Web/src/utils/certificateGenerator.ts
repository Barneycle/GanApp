...
async function loadGoogleFontForCanvas(fontFamily: string, fontWeight: string = '400'): Promise<boolean> {
  ...
  try {
    ...
    const _testFont = new FontFace(fontFamilyClean, 'normal');
...
async function loadCertificateFonts(config: any): Promise<void> {
  ...
  if (config.signature_blocks) {
    config.signature_blocks.forEach((sig: any) => {
      ...
      if (sig.typed_font) {
        fontsToLoad.add(sig.typed_font);
        fontWeights.set(sig.typed_font, '400');
      }
    });
  }
...
export async function generatePNGCertificate(
  ...
  for (const signature of signatures) {
    ...
    if (signature.signature_source === 'type' && (signature.typed_text || '').trim()) {
      const typedFont = signature.typed_font || 'Dancing Script, cursive';
      const typedSize = Math.max(18, (signature.signature_image_width || 300) * 0.2);
      ctx.fillStyle = '#111827';
      ctx.font = `400 ${typedSize}px ${typedFont}`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText((signature.typed_text || '').trim(), sigX, sigY - 8);
    } else if (signature.signature_image_url) {
      const imgWidth = signature.signature_image_width || 280;
      const imgHeight = signature.signature_image_height || Math.round(imgWidth / 3);
      await drawImage(
